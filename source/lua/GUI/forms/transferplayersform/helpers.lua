...
function update_transfers_page()
    TransferPlayersForm.TransfersPageLabel.Caption = string.format(
        'Page: %d', CUSTOM_TRANSFERS_PAGE
    )
end

function has_next_transfers_page(max_transfers)
    if (CUSTOM_TRANSFERS_PAGE * MAX_TRANSFERS_PER_PAGE) < max_transfers then
        return true
    else
        return false
    end
...
function has_prev_transfers_page()
    if CUSTOM_TRANSFERS_PAGE <= 1 then
        return false
    else
        return true
    end
...
function fill_custom_transfers()
    ...
    local idx = (CUSTOM_TRANSFERS_PAGE-1) * MAX_TRANSFERS_PER_PAGE
    local max_comps = idx + MAX_TRANSFERS_PER_PAGE
    ...
    if has_next_transfers_page(num_of_transfers) then
        TransferPlayersForm.NextTransfersPageBtn.Enabled = true
    else
        TransferPlayersForm.NextTransfersPageBtn.Enabled = false
    end

    if has_prev_transfers_page() then
        TransferPlayersForm.PrevTransfersPageBtn.Enabled = true
    else
        TransferPlayersForm.PrevTransfersPageBtn.Enabled = false
    end
    update_transfers_counter(num_of_transfers)
end
...
function new_custom_transfer(playerid, current_team_ids, teamid, clause, contract_length, is_confirmed_by_user)
    ...
    local player = {
        playerid = playerid,
        skintonecode = tonumber(ADDR_LIST.getMemoryRecordByID(CT_MEMORY_RECORDS['SKINTONECODE']).Value),
